...
def enhance_with_gamma_correction(
    image: Image.Image, gamma: float = 2.2
) -> Image.Image:
...
def enhance_with_msrcr(
    ...
    MSRCR extends MSR by adding a color restoration function that preserves
    and enhances color information. It provides the best overall performance
    among Retinex methods for low-light enhancement, with good detail
    enhancement and natural color rendition.

    Args:
        image: PIL Image (RGB)
        sigmas: Tuple of standard deviations for Gaussian filters
    ...
    """
    img_array = np.array(image)
    img_enhanced = msrcr(img_array, sigmas=sigmas)
    # Clip values to valid range and convert to uint8
    img_output = np.clip(img_enhanced, 0, 255).astype(np.uint8)
    return Image.fromarray(img_output)
...
CLASSICAL_METHODS = {
    ...
    "msrcr": {
        "name": "MSRCR",
        "function": enhance_with_msrcr,
        "description": "Multi-Scale Retinex with Color Restoration",
    },
}
