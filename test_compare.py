...
if test_zero_dce:
    ...
    try:
        ...
        print(
            f"   ✅ Output value range: [{enhanced_array.min()}, {enhanced_array.max()}]"
        )
    except Exception as e:
...
try:
    ...
    if not os.path.exists(output_path):
        print("   ❌ Comparison output not created")
        sys.exit(1)

    # Check file size
    file_size = os.path.getsize(output_path)
    if file_size == 0:
        print("   ❌ Comparison output is empty")
        sys.exit(1)
...
if test_reference:
    try:
        ...
        # Check output file exists
        if not os.path.exists(output_path):
            print("   ❌ Comparison with reference not created")
            sys.exit(1)
...
if test_zero_dce:
    ...
    if test_reference:
        print(
            f"  python compare.py -i {test_image_path} -w {weights_path} -r {reference_image_path} -o output.png"
        )
else:
