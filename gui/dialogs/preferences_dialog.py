...
class GeneralTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.weights_dir_edit.setPlaceholderText(
        ...
        self.zoom_mode_combo.setToolTip("Default zoom mode for newly loaded images")
        ...
        self.max_dimension_combo.currentIndexChanged.connect(
            self._on_max_dimension_changed
        )
    ...
    def _browse_weights_directory(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Select Weights Directory", start_dir, QFileDialog.Option.ShowDirsOnly
        )
...
class PreferencesDialog(QDialog):
    ...
    def _connect_change_signals(self):
        ...
        self.general_tab.auto_load_checkbox.stateChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.zoom_mode_combo.currentIndexChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.sync_zoom_checkbox.stateChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.show_info_checkbox.stateChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.show_histogram_checkbox.stateChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.histogram_type_combo.currentIndexChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.gpu_mode_combo.currentIndexChanged.connect(
            self._on_settings_changed
        )
        self.general_tab.max_dimension_combo.currentIndexChanged.connect(
            self._on_settings_changed
        )
