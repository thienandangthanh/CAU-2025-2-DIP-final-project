    ...
    def run_multiple_methods(
        ...
        for idx, method_key in enumerate(method_keys, start=1):
            ...
            if self._should_cancel:
                if emit_signals:
                    self.progress_updated.emit(idx - 1, total_methods, "Cancelled")
                break
            ...
            if emit_signals:
                method_info = self.registry.get_method_info(method_key)
                progress_msg = (
                    f"Processing {method_info.name}... ({idx}/{total_methods})"
                )
    ...
    def run(self):
        ...
        try:
            self.runner.run_multiple_methods(
                self.image, self.method_keys, self.model, emit_signals=True
            )
        except Exception as e:
...
def enhance_with_methods(
    image: Image.Image, method_keys: list[str], model: ZeroDCE | None = None
) -> dict[str, EnhancementResult]:
