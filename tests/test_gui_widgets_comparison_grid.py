...
class TestComparisonGrid:
    ...
    def test_set_methods_without_reference(self, qapp):
        ...
        grid.set_methods(method_keys, method_names, show_input=True, show_reference=False)
    ...
    def test_set_methods_without_input(self, qapp):
        ...
        grid.set_methods(method_keys, method_names, show_input=False, show_reference=False)
    ...
    def test_get_all_cells(self, qapp):
        ...
        grid = ComparisonGrid()
        grid.set_methods(["method1", "method2"], {"method1": "M1", "method2": "M2"}, show_input=True)
...
class TestComparisonGridIntegration:
    ...
    def test_optimal_columns_by_cell_count(self, qapp):
        ...
        grid.set_methods(["m1", "m2", "m3"], {"m1": "M1", "m2": "M2", "m3": "M3"}, show_input=True)
        assert grid.columns == 2
        
        # Test 5 cells: should be 3 columns (2x3)
        grid.set_methods(["m1", "m2", "m3", "m4"], {"m1": "M1", "m2": "M2", "m3": "M3", "m4": "M4"}, show_input=True)
        assert grid.columns == 3
        
        # Test 6 cells: should be 3 columns (2x3)
        grid.set_methods(["m1", "m2", "m3", "m4", "m5"], {"m1": "M1", "m2": "M2", "m3": "M3", "m4": "M4", "m5": "M5"}, show_input=True)
        assert grid.columns == 3
        
        # Test 7+ cells: should be 4 columns
        grid.set_methods(["m1", "m2", "m3", "m4", "m5", "m6"], {"m1": "M1", "m2": "M2", "m3": "M3", "m4": "M4", "m5": "M5", "m6": "M6"}, show_input=True)
        assert grid.columns == 4
    ...
    def test_uniform_stretch_factors(self, qapp):
        ...
        for col in range(grid.columns):
            ...
            assert stretch == 1, f"Column {col} should have stretch factor 1, got {stretch}"
        ...
        for row in range(num_rows):
            ...
            assert stretch == 1, f"Row {row} should have stretch factor 1, got {stretch}"
