...
class TestEnhancementRunnerThread:
    ...
    def test_thread_signals_emitted(self, sample_image, qtbot):
        ...
        thread.method_started.connect(lambda *args: signal_counts.update({"started": signal_counts["started"] + 1}))
        thread.method_completed.connect(lambda *args: signal_counts.update({"completed": signal_counts["completed"] + 1}))
        thread.all_completed.connect(lambda *args: signal_counts.update({"all": signal_counts["all"] + 1}))
