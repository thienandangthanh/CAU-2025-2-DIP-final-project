...
try:
    ...
    print(f"   ✅ Test image created: {test_image.size} pixels, mode: {test_image.mode}")
except Exception as e:
...
try:
    ...
    print(f"   ✅ Custom parameters work: clip_limit=3.0, tile_size=16")
except Exception as e:
...
try:
    ...
    print(f"   ✅ Custom gamma works: gamma=1.5")
except Exception as e:
...
if Path(real_image_path).exists():
    try:
        ...
        print(f"   ✅ All methods work on real image")
    except Exception as e:
        print(f"   ❌ Real image test failed: {e}")
        sys.exit(1)
else:
    print(f"   ⚠️  Skipping (real image not available)")
print()

# Test 9: Test via CLASSICAL_METHODS dictionary
print("9. Testing method invocation via CLASSICAL_METHODS...")
try:
    for method_key, method_info in CLASSICAL_METHODS.items():
    ...
    print(f"   ✅ All methods callable via CLASSICAL_METHODS dictionary")
except Exception as e:
